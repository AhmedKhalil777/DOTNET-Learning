        ...
        private void ProcessMessage(byte[] messageBytes, ref object returnVal, ref bool shouldCloseConnection)
        {
            ...
            switch (command.Command)
            {
                    ...
                    if (command.Type != null)
                    {
                        ...
                        try
                        {
                            if (command.Parameters?.Length > 0)
                            {
                                ...
                                {
                                    ctorParameters[i] = SerializationHelper.GetObjectAsType(command.Parameters[i], command.ParametersTypes[i]);
                                }
                ...
                case Commands.Invoke:
                    ...
                    {
                        // Static method
                        method = command.Type.GetRuntimeMethod(command.MemberName, command.ParametersTypes);
                    }
                    else if (command.ObjectId != Guid.NewGuid())
                    {
                        ...
                        if (Objects.TryGetValue(command.ObjectId, out obj))
                        {
                            ...
                            method = objectToInvokeOn?.GetType().GetRuntimeMethod(command.MemberName, command.ParametersTypes);
                        }
                    }
                    ...
                    if (command.Parameters?.Length > 0)
                    {
                        ...
                        for (int i = 0; i < command.Parameters.Length; i++)
                        {
                            parameters[i] = SerializationHelper.GetObjectAsType(command.Parameters[i], command.ParametersTypes[i]);
                        }
                    }
                ...
                case Commands.SetProperty:
                    ...
                    if (command.Parameters?.Length > 0)
                    {
                        parameter = SerializationHelper.GetObjectAsType(command.Parameters[0], command.ParametersTypes[0]);
                    }
